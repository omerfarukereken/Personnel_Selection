...
def anaprogram():
    ...
    if secim == "Başvuru Girişi":
        ...
        dogumtarihi = st.date_input("Doğum Tarihiniz", min_value=datetime.date(1980, 1, 1),
                                    max_value=datetime.date(2000, 1, 1))
        ...
        if referans_yoktur+referans_vardır>1:
            st.warning("Lütfen Tek Kutucuğu Seçiniz")
            cift_engelleyici = 1
        if referans_vardır == 1:
            referans = st.text_input('Şirketimizdeki Referansınız Kimdir?:')
        ###################################################
        a, b, c = st.beta_columns(3)
        with a:
        ...
        model = pickle.load(open("modelim", "rb"))
        ...
        with d:
            if st.button("Tamamla") and cift_engelleyici!=1:
                yz_haric = {"isim":isim, "soyisim":soyisim, "dogumtarihi":dogumtarihi,"ikametili":ikametili,
                            ...
                            "yükseklisansokul":yükseklisansokul, "doktoraokul":doktoraokul, "referans":referans}
        ...
        if st.sidebar.checkbox("Giriş"):
            if sifre == "":
                ...
                ("Lojistik Regresyon", "K En Yakın Komşular", "Destek Vektör Makineleri", "Karar Ağacı", "Rastgele Orman", "XGBoost"), key="d")
                ...
                def parametre_belirle(secilen_model):
                    ...
                    if secilen_model == "K En Yakın Komşular":
                        ...
                        max_depth = st.sidebar.slider("Maksimum Derinlik", 2, 15)
                        n_estimators = st.sidebar.slider("Ağaç Sayısı", 1, 100)
                ...
                def modeli_calıstır(secilen_model, parametreler):
                    ...
                    elif secilen_model == "Destek Vektör Makineleri":
                        modelim = SVC()
                ...
                if secilen_model=="Lojistik Regresyon" or secilen_model=="K En Yakın Komşular" or secilen_model=="Destek Vektör Makineleri":
                    normalize = StandardScaler()
                    X_egitim = normalize.fit_transform(X_egitim)
                    X_test = normalize.transform(X_test)
                    X = normalize.transform(X)
                    try:
                        basvuru_degerlendirme = normalize.transform(basvuru_degerlendirme)
                    except:
                        pass
